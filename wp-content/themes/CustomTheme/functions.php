...
if (is_page_template('page-template/store/store-listing.php'))
{
    include_once 'page-templates/store/store-listing.php';
}
...
function wp_bootstrap_starter_widgets_init() {
    ...
    register_sidebar( array(
        'name'          => esc_html__( 'Header Cat', 'wp-bootstrap-starter' ),
        'id'            => 'header-cat',
        'description'   => esc_html__( 'Add widgets here.', 'wp-bootstrap-starter' ),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget'  => '</section>',
    ...
    register_sidebar( array(
        'name'          => esc_html__( 'Search Widget', 'wp-bootstrap-starter' ),
        'id'            => 'search-widget',
        'description'   => esc_html__( 'Add widgets here.', 'wp-bootstrap-starter' ),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget'  => '</section>',
...
function wp_bootstrap_starter_scripts() {
	...
	// load bootstrap js
    wp_enqueue_script('owl-carousel.min.js', get_template_directory_uri() . '/inc/assets/js/owl.carousel.min.js', array(), '', true );
    if ( get_theme_mod( 'cdn_assets_setting' ) === 'yes' ) {
...
function update_user(){
    if (isset( $_POST["user_update_nonce"] ) && wp_verify_nonce($_POST['user_update_nonce'], 'user_update_nonce')) {
        
        $user_cstm_id  = $_POST["user_cstm_id"];
        ...
        update_field( 'address', $user_address , 'user_'.$user_cstm_id );
        update_field( 'phone_number',  $user_phone , 'user_'.$user_cstm_id );
        update_field( 'file',  $user_img , 'user_'.$user_cstm_id );
        ...
        if ( $movefile && !isset($movefile['error']) ) {
            ...
            // update_field('zipcode', $zipcode, $post_id);
            // update_field('date', $date, $post_id);
    }
}
add_action('init','update_user');
...
function add_data_in_crm_database(){
    ...
    if (isset( $_POST["user_add_nonce"] ) && wp_verify_nonce($_POST['user_add_nonce'], 'user_add_nonce')) {
        ...
        update_field('address' , $useraddress , 'user_'. $user_id);
        update_field('phone_number' , $userphone , 'user_'. $user_id);
        
        wp_redirect('http://127.0.0.1/bridget/user-management/' );
        exit;

    }
}
add_action('init','add_data_in_crm_database');
...
function login_ban_status($user) {
        ...
        $status = get_field('ban_user','user_'.$user->ID);
...
add_action( 'wcfmmp_new_store_created', function( $member_id, $wcfmmp_settings ) {
    ...
    update_field('ban_user' , 1 , 'user_'. $member_id);

}, 50, 2 );

function groupAddProfile() {

    if (isset( $_POST["category_add_nonce"] ) && wp_verify_nonce($_POST['category_add_nonce'], 'category_add_nonce')) {
    ...
    $cat_status          = $_POST["cat_status"];
            ...
            update_field( 'status', $cat_status , $pos_type_id );
...
add_action('init', 'groupAddProfile');
