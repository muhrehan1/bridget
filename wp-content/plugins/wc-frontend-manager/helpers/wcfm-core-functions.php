...
function wcfm_get_option( $key, $default_val = '', $lang_code = '' ) {
	...
	if ( defined( 'ICL_SITEPRESS_VERSION' ) && ! ICL_PLUGIN_INACTIVE && class_exists( 'SitePress' ) ) {
		...
		$option_val_wpml = get_option( $key . '_' . $current_language );

		if( $option_val_wpml ) {
			$option_val = $option_val_wpml;
		} else {
			$default_language = $sitepress->get_default_language();
			$default_val_wpml = get_option( $key . '_' . $default_language );

			if( $default_val_wpml ) {
				$option_val = $default_val_wpml;
			}
		}
	}
...
function wcfm_get_post_meta( $post_id, $key, $is_single = true ) {
	...
	if ( defined( 'ICL_SITEPRESS_VERSION' ) && ! ICL_PLUGIN_INACTIVE && class_exists( 'SitePress' ) ) {
		...
		$current_language = $sitepress->get_current_language();
		$meta_val_wpml = get_post_meta( $post_id, $key . '_' . $current_language, $is_single );

		if( $meta_val_wpml ) {
			$meta_val = $meta_val_wpml;
		} else {
			$default_language = $sitepress->get_default_language();
			$default_val_wpml = get_post_meta( $post_id, $key . '_' . $default_language, $is_single );

			if( $default_val_wpml ) {
				$meta_val = $default_val_wpml;
			}
		}
	}
...
function wcfm_get_user_meta( $user_id, $key, $is_single = true ) {
	...
	if ( defined( 'ICL_SITEPRESS_VERSION' ) && ! ICL_PLUGIN_INACTIVE && class_exists( 'SitePress' ) ) {
		...
		$current_language = $sitepress->get_current_language();
		$meta_val_wpml = get_user_meta( $user_id, $key . '_' . $current_language, $is_single );

		if( $meta_val_wpml ) {
			$meta_val = $meta_val_wpml;
		} else {
			$default_language = $sitepress->get_default_language();
			$default_val_wpml = get_user_meta( $user_id, $key . '_' . $default_language, $is_single );
